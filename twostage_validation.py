...
function show(n) {{
  ...
  const _pane = document.getElementById('pane-' + n);
  _pane.classList.add('a');
  // Force layout reflow: reading offsetHeight causes the browser to flush pending
  // style changes (display:none→block) before Chart.js measures canvas dimensions.
  // Without this, Chart.js sees 0×0 and renders nothing.
  void _pane.offsetHeight;
  if (_INIT[n] && !_DONE[n]) {{
    try {{ _INIT[n](); }} catch(e) {{ console.error('chart init error on tab', n, ':', e); }}
    _DONE[n] = true;
  }}
