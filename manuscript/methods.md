...
patient and attempt rather than to call order (§3.8), any difference in outcome
...
> S_u(t) in a survival-augmented network-design MILP extending i-SHIPMENT. Its
> solution fixes four quantities — open facilities, capacity, patient-to-facility
> assignment and transport modes — which pass to Layer 2 as a frozen network and
> are never re-decided there. **Layer 2 (operational, online, over simulated
> time)** advances a daily discrete-event clock (1) on which patients arrive and
> material is staged. A decision epoch (2) is triggered whenever a manufacturing
> slot frees; the per-epoch rolling-horizon problem (P1)–(P8) (3) then reads the
...
> projected survival at the remake's delivery is at least S_min, returning the
> patient to the queue with a larger accrued wait, and otherwise cancelled. The
> clock then advances one day. The five policies evaluated (FIFO, survival-index,
> static survival, adaptive MPC, and the perfect-information bound) enter at the
> per-epoch decision point (3) and differ only there: each is executed on the
> identical frozen network and, through common random numbers, the identical
> realised failures.
...
S_p ∈ [0,1]; the queue delay HOLD_p ≥ 0; and the material arrival A_pmt.
...
INM is released as a decision, governed by the queue (32)–(34) below.

Concurrent capacity is enforced through (13)–(15), and the network structure —
at most two open facilities, links only to open facilities, exactly one journey
per leg, and delivery to the hospital co-located with the collection site —
through (16)–(23), unchanged from i-SHIPMENT. Timing follows (24)–(26), with

  TRT_p = CTT_p − STT_p,   STT_p ≤ CTT_p,                        ∀p,   (25)

the turnaround time of patient p, measured from collection to infusion.

### 3.3.3 The manufacturing queue

Replacing the fixed start with a queue requires four constraints:

  Σ_{τ≤t} INM_pmτ ≤ Σ_{τ≤t} A_pmτ,                               ∀p,m,t,   (32)
  Σ_t INM_pmt = Σ_t A_pmt,                                       ∀p,m,     (33)
  Σ_p DURV_pmt ≤ FCAP_m,                                         ∀m,t,     (34)
  HOLD_p = Σ_{m,t} t (INM_pmt − A_pmt) ≥ 0,                      ∀p.       (35)

Constraint (32) forbids a job from starting before its material has arrived;
(33) requires each routed material to be started exactly once; (34) caps
concurrent manufacturing at each site; and (35) defines the hold, the delay
...
makes the design identifiable. Capacity (34) forces some patients to wait; the
...
The survival model is
  ...
  S_p = exp( −κ · HR_u(p) · (TRT_p / η)^γ ),                     ∀p,       (27)
...
binary indicators δ_{p,d}, d ∈ {17, …, 42}, and impose
  ...
  Σ_d δ_{p,d} = 1,   TRT_p = Σ_d d · δ_{p,d},   S_p = Σ_d σ_{d,u(p)} δ_{p,d},  ∀p, (28′–30′)

with σ_{d,u} = S_u(d) precomputed from (27). Because the support of TRT_p is
exactly the breakpoint set, (28′–30′) is not a piecewise-linear approximation:
it reproduces (27) exactly at every feasible point. This is a strengthening of
the SOS2 λ-method stated as (28)–(30) in the model specification, which the
...
*Aggregate throughput cuts.* Constraint (34) admits at most FCAP_m starts in any
...
ends by T). These are implied by (34) and therefore change neither the feasible
...
S_H = 0.936 against S_L = 0.992 — a 5.6-point gap that widens to 13 points at 42
...
Survival is evaluated from the observed state, not from a nominal plan:
  ...
  elapsed_i(τ) = a_i + (τ − t) + T_MFE + T_QC + TT3_{m(i)},                  (P1)
  S_i(τ) = (1 − w_{u(i)})^{elapsed_i(τ)/42}.                                 (P2)

Because the survival clock runs continuously from the *first* collection,
a_i = t − t⁰_i, and (P1) simplifies to elapsed_i(τ) = τ − t⁰_i + T_MFE + T_QC +
...
The epoch problem is
  ...
  min Σ_{i∈W_t} ρ_{u(i)} (1 − Ŝ_i),                                          (P3)

with effective survival

  Ŝ_i = Σ_τ S_i(τ) x_{i,τ} + d_i (1 − Σ_τ x_{i,τ}),   d_i = S_i(t+H),

subject to

  Σ_τ x_{i,τ} ≤ 1,                                        ∀i ∈ W_t,          (P4)
  Σ_{i: m(i)=m} Σ_{τ−T_MFE<τ′≤τ} x_{i,τ′} ≤ FCAP_m − busy_m(τ),  ∀m, τ ∈ [t, t+H], (P5)
  x_{i,τ} = 0                                             for τ < t.         (P6)

The continuation term d_i is what prevents (P3) from degenerating into a myopic
rule: deferring a patient past the window is charged at the survival that
deferral would cost, so the model weighs serving a patient now against the
damage of not serving them at all within the horizon. The frozen operational
cost c^op_i is inert on a frozen network — it is a per-patient constant and every
started patient is started exactly once — so it does not enter the argmin and is
not monetised into (P3); α appears only in the strategic objective (1).

Two structural properties are worth stating. First, because m(i) is frozen, the
patient set partitions across facilities and (P5) never couples two of them: the
...
(P3)–(P6) collapse to a closed form:

  i* = argmax_{i∈W_t} ρ_{u(i)} [ S_i(t) − S_i(t+1) ].                        (P8)
...
- **Feasibility.** Re-collection is admissible if and only if projected survival
  ...
  Ŝ_i(delivery) = (1 − w_{u(i)})^{[a_i + (T_LS + TT1) + wait_i + T_MFE + T_QC + TT3]/42} ≥ S_min, (37)

  where wait_i is the days until the next free slot at m(i), read from the same
  occupancy busy_m(·) that (P5) uses. Because the floor is evaluated on
...
horizon. §3.9 documents why this choice is material.
...
| `survival_index` | The closed-form index (P8). |
...
| `adaptive_mpc` | The full per-epoch model (P1)–(P6), re-solved at every epoch on the observed state. |
...
starts, Σ_i x_{i,t} = min(free slots at t, |W_t|), so that policies differ in
*whom* they select and never in *how much* capacity they use — without it, a
policy could appear to reduce clinical loss simply by withholding capacity.
...
attempt's release plus a re-collection,
  ...
  Σ_τ x_{i,k+1,τ} ≤ Σ_τ x_{i,k,τ},
  Σ_τ τ x_{i,k+1,τ} ≥ Σ_τ τ x_{i,k,τ} + T_MFE + T_QC + T_LS + TT1 − M(1 − Σ_τ x_{i,k+1,τ}),  (38)

subject to the same rolling-window capacity the simulation enforces and the same
S_min admissibility. The objective maximises realised weighted survival, in which
only a successful attempt delivers anything:

  max Σ_i Σ_{k: f_{i,k}=0} Σ_τ ρ_{u(i)} S_{u(i)}(τ + T_MFE + T_QC + TT3_i − t⁰_i) x_{i,k,τ}.  (39)
...
this benchmark is deliberately exempt from the non-idling requirement of §3.7:
...
*The re-collection floor is weakly binding.* In (37) the term wait_i is read
...
time, MIP gap and model size are recorded for every solve. Every replication records the seed that produced it, the strategic
solution it was frozen against, and the resulting per-experiment metrics, so
that any reported number can be traced to a specific solve and a specific draw.
Code, instance data, cached strategic solutions and all per-experiment outputs
