"""
CAR-T Supply Chain — Column Generation (Flexible Wait)
=======================================================
Identical to cart_colgen.py except that patients are allowed to WAIT
at the manufacturing facility before manufacturing begins.

Maximum wait per patient is determined by remaining deadline slack:
    max_wait = D_p - (TLS + TT1[j] + TMFE + TQC + TT3[k])

This gives the solver scheduling flexibility to spread capacity load
across time and avoid artificial infeasibility caused by clustered arrivals.

A "plan" for patient p = (facility m, outbound mode j, return mode k, wait w)
  - TRT = TLS + TT1[j] + TMF + TQC + TT3[k] + w
  - Hard constraint: TRT <= D_p  (w uses all available slack)
        ...
        for mi in M:
            for j_out in J:
                for j_ret in J:
                    ...
                    # Allow patient to wait up to the full remaining deadline slack.
                    # This lets the solver shift manufacturing start times to avoid
                    # capacity bottlenecks caused by clustered patient arrivals.
                    max_wait = int(dl) - base_trt  # >= 0 by the check above
                    ...
                    # Generate one plan per valid wait value (0 … max_wait)
...
run_experiment_flexwait = run_experiment
