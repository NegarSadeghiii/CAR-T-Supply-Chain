...
> `TECHNICAL_APPENDIX.md §4b`). The numbers below use the earlier calibration and
> are superseded by the `paper/` assets (and its publication figures `F1`–`F7`)
> for exact values; the qualitative conclusions (large prediction gap; a
> survival-based priority rule roughly halves high-urgency losses) are unchanged.
...
network runs the other way — most of
...
*Reproduce:* `python run_all.py`. The publication figures for this analysis are
generated by the manuscript pipeline in `paper/` (`python paper/make_assets.py`,
figures `F1`–`F7`). Validation checks and internal model diagnostics (in
technical language) are in `TECHNICAL_APPENDIX.md`.
